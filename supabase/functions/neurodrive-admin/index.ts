...
function getVerifiedUserIdFromToken(token: string): string | null {
  try {
...
Deno.serve(async (req: Request) => {
  ...
  // The Supabase Edge Gateway enforces verify_jwt=true before this code runs.
  // Decode the already-verified JWT only to obtain the authenticated subject.
  ...
  const instructorLessonUsers = new Set(lessonRows.filter((lesson) => instructorIds.has(lesson.user_id)).map((lesson) => lesson.user_id));
  ...
  return json({
    ...
    totals: { assessments: assessments.length, scored_assessments: validScores.length, average_score: averageScore, ufs: ufDistribution.length },
    uf_distribution: ufDistribution,
    risk_distribution: riskDistribution,
    recent: assessments.slice(0, 10).map((r) => ({ observed_at: r.observed_at, uf: r.uf, age_band: r.age_band, cnh_category: r.cnh_category, instrument: r.instrument, instrument_version: r.instrument_version, total_score: r.total_score, risk_class: r.risk_class })),
    instructor_intelligence: {
      ...
      profile_completion_rate: instructorProfiles.length ? Number((completeProfiles / instructorProfiles.length * 100).toFixed(1)) : 0,

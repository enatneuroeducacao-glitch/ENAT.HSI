...
import { ConfiguracaoPortalENAT } from "./pages/ConfiguracaoPortalENAT";
...
function PublicArea(){return <Routes><Route path="/" element={<PortalPublicoENAT/>}/><Route path="/portal" element={<PortalPublicoENAT/>}/><Route path="/validar/:code" element={<ValidarCertificadoPublico/>}/><Route path="/login" element={<LoginENAT/>}/></Routes>}
function PrivateArea(){const {session,profile,loading}=useAuth();const location=useLocation();if(loading)return <main style={{minHeight:"100vh",display:"grid",placeItems:"center",background:"#07111b",color:"#63caff",fontFamily:"Arial"}}>Validando autorização…</main>;if(!session||!profile)return <Navigate to="/login" replace state={{from:location.pathname}}/>;return <><MenuENAT/><Routes><Route path="/dashboard-enat" element={<DashboardENAT/>}/><Route path="/home-admin" element={<Home/>}/><Route path="/sobre" element={<SobreENAT/>}/><Route path="/produtos" element={<ProdutosENAT/>}/><Route path="/produtos/:produto" element={<ProdutoDetail/>}/><Route path="/certificacao" element={<CertificacaoENAT/>}/><Route path="/governanca" element={<GovernancaENAT/>}/><Route path="/contato" element={<ContatoENAT/>}/><Route path="/resultados" element={<ResultadosENAT/>}/><Route path="/cadastro/instrutor" element={<CadastroInstrutor/>}/><Route path="/cadastro/aluno" element={<CadastroAluno/>}/><Route path="/cursos/admin" element={<BancoCursosENAT/>}/><Route path="/cursos" element={<CursosENAT/>}/><Route path="/cursos/:courseId" element={<CursoDetalhado/>}/><Route path="/testes" element={<TestesAvancados/>}/><Route path="/testes/:testId" element={<TesteAprofundado/>}/><Route path="/certificados" element={<Certificados/>}/><Route path="/dashboard" element={<DashboardAluno/>}/><Route path="/relatorio-turmas" element={<RelatorioTurmas/>}/><Route path="/indicacoes" element={<IndicacoesInstrutor/>}/><Route path="/emissor" element={<EmissorCertificadosComBusca/>}/><Route path="/docente-enat" element={<DocenteENAT/>}/><Route path="/recuperacao" element={<RecuperacaoCentralENATHSI/>}/><Route path="/alunos" element={<BancoAlunosENAT/>}/><Route path="/caixa-publica" element={<CaixaEntradaPublicaENAT/>}/><Route path="/configuracao-portal" element={<ConfiguracaoPortalENAT/>}/><Route path="/usuarios" element={profile.role==="admin"?<UsuariosENAT/>:<Navigate to="/emissor" replace/>}/><Route path="*" element={<Navigate to="/dashboard-enat" replace/>}/></Routes></>}
function PublicAdminBridge(){useEffect(()=>{const handler=e=>{const el=e.target?.closest?.(".admin-entry,.portal-footer button");if(!el)return;e.preventDefault();const base=window.location.hostname.endsWith("github.io")?"/ENAT.HSI":"";window.location.assign(`${base}/login`)};document.addEventListener("click",handler);return()=>document.removeEventListener("click",handler)},[]);return null}

...
function PrivateArea() {
  ...
  if (loading) {
    return <main style={{ minHeight: "100vh", display: "grid", placeItems: "center", background: "#07111b", color: "#63caff", fontFamily: "Arial" }}>Validando autorização…</main>;
  }
  ...
  return (
    <>
      <MenuENAT />
      <Routes>
        <Route path="/dashboard-enat" element={<DashboardENAT />} />
        ...
        <Route path="/cursos/admin" element={<BancoCursosENAT />} />
        ...
        <Route path="/emissor" element={<EmissorCertificadosComBusca />} />
        <Route path="/docente-enat" element={<DocenteENAT />} />
        <Route path="/recuperacao" element={<RecuperacaoCentralENATHSI />} />
        <Route path="/alunos" element={<BancoAlunosENAT />} />
        <Route path="/caixa-publica" element={<CaixaEntradaPublicaENAT />} />
        <Route path="/configuracao-portal" element={<ConfiguracaoPortalENAT />} />
        <Route path="/usuarios" element={profile.role === "admin" ? <UsuariosENAT /> : <Navigate to="/emissor" replace />} />
        <Route path="*" element={<Navigate to="/dashboard-enat" replace />} />
      </Routes>
    </>
  );
}
...
export default function CentralENATHSI() {
  return (
    <BrowserRouter basename={APP_BASENAME}>
      <AuthProvider>
        <Routes>
          <Route path="/" element={<PortalPublicoENAT />} />
          <Route path="/portal" element={<PortalPublicoENAT />} />
          <Route path="/login" element={<LoginENAT />} />

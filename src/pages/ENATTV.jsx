  ...
  return <main className="enat-tv-page">
    ...
    {showSubmission && <div className="enat-tv-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setShowSubmission(false); }}>
      <section className="enat-tv-submit-modal" role="dialog" aria-modal="true" aria-labelledby="tv-submission-title">
        ...
        <p>Sugira entrevistas, notícias, eventos, pesquisas, vídeos ou materiais educacionais para análise da Central Editorial.</p>
        <form onSubmit={submit} className="enat-tv-form"><div className="enat-tv-form-grid"><input required placeholder="Nome" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /><input required type="email" placeholder="E-mail" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} /></div><input required placeholder="Assunto / pauta" value={form.subject} onChange={(e) => setForm({ ...form, subject: e.target.value })} /><textarea required rows="5" placeholder="Descreva a pauta ou material" value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} /><label className="enat-tv-file">📎 Anexar arquivos <input type="file" multiple onChange={handleFiles} /></label>{files.length > 0 && <small>{files.length} arquivo(s) selecionado(s)</small>}<label className="enat-tv-consent"><input type="checkbox" checked={consent} onChange={(e) => setConsent(e.target.checked)} required /> Concordo com o uso dos dados informados para análise e contato sobre esta submissão.</label><div className="enat-tv-modal-actions"><button type="button" className="enat-tv-secondary" onClick={() => setShowSubmission(false)}>Cancelar</button><button className="enat-tv-primary" disabled={sending} type="submit">{sending ? "Enviando…" : "Enviar para a redação"}</button></div>{status && <p role="status" className="enat-tv-form-status">{status}</p>}</form>
      </section>
    </div>}
  </main>;
}

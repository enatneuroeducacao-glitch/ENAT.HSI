...
export function ENATTV() {
  ...
  const [files, setFiles] = useState([]); const [consent, setConsent] = useState(false); const [status, setStatus] = useState(""); const [sending, setSending] = useState(false); const [editorial, setEditorial] = useState([]); const [mediaUrls, setMediaUrls] = useState({});

  useEffect(() => {
    const loadEditorial = async () => {
      if (!supabase) return;
      const { data } = await supabase.from("enat_public_editorial").select("id,title,summary,content,category,image_url,tags,featured,published,published_at,metadata").eq("published", true).order("featured", { ascending: false }).order("published_at", { ascending: false });
      const rows = data || []; setEditorial(rows);
      const mediaRows = rows.filter((item) => item?.metadata?.media?.path);
  ...
  const handleFiles = (event) => {
    const selected = Array.from(event.target.files || []); const invalidType = selected.find((file) => !ALLOWED_EXTENSIONS.has(file.name.split(".").pop()?.toLowerCase() || ""));
    if (invalidType) { setFiles([]); setStatus(`Formato não permitido: ${invalidType.name}.`); event.target.value = ""; return; }
    const oversized = selected.find((file) => file.size > MAX_FILE_SIZE); if (oversized) { setFiles([]); setStatus(`O arquivo ${oversized.name} excede o limite de 15 MB.`); event.target.value = ""; return; }
  ...
  const submit = async (event) => {
    event.preventDefault(); if (!consent) { setStatus("É necessário concordar com o uso dos dados para análise e contato sobre esta submissão."); return; }
    if (!supabase) { setStatus("Canal de recebimento não configurado."); return; }
    setSending(true); setStatus("Enviando material…");
    try {
      const data = new FormData(); Object.entries({ ...form, kind: "tv_submission", consent: "true" }).forEach(([key, value]) => data.append(key, value)); files.forEach((file) => data.append("files", file, file.name));
      const endpoint = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/enat-public-inbox`; const response = await fetch(endpoint, { method: "POST", headers: { apikey: import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY }, body: data }); const result = await response.json().catch(() => ({}));
    ...
    } catch (error) { setStatus(error.message || "Falha no envio."); } finally { setSending(false); }
  };
  ...
  return (
    <main className="enat-tv-page">
      ...
      <section className="enat-tv-grid" aria-label="Programação da ENAT TV">
        {PROGRAMS.map((program) => <Link key={program.key} to={`/tv?categoria=${encodeURIComponent(program.key)}`} style={{ textDecoration: "none", color: "inherit" }} aria-label={`Abrir ${program.key}`}><article><span>{program.icon}</span><h2>{program.key}</h2><p>{program.description}</p><small>Ver conteúdos →</small></article></Link>)}
      </section>
      <section className="enat-tv-editorial" aria-labelledby="tv-editorial-title"><div><span className="enat-tv-eyebrow">PUBLICAÇÕES</span><h2 id="tv-editorial-title">{selectedCategory ? `Conteúdos: ${selectedCategory}` : "Conteúdos publicados pela redação"}</h2><p>Materiais aprovados e publicados pela Central Editorial da ENAT TV.</p>{selectedCategory && <Link to="/tv">← Ver todas as publicações</Link>}</div>
        <div className="enat-tv-editorial-list">{filteredEditorial.map((item) => { const media = mediaUrls[item.id]; const isVideo = media?.type?.startsWith("video/"); return <article key={item.id} className="enat-tv-editorial-card">{item.image_url && <img src={item.image_url} alt="" loading="lazy" />}{isVideo && <video controls playsInline preload="metadata" src={media.url} style={{ width: "100%", maxHeight: "420px", borderRadius: "12px", background: "#000" }}>Seu navegador não conseguiu reproduzir este vídeo.</video>}<div>{item.category && <span className="enat-tv-eyebrow">{item.category}</span>}<h3>{item.title}</h3>{item.summary && <p>{item.summary}</p>}<small>{item.published_at ? new Date(item.published_at).toLocaleDateString("pt-BR") : "Publicado pela ENAT TV"}</small></div></article>; })}{!filteredEditorial.length && <div className="enat-tv-empty">{selectedCategory ? "Ainda não há publicações nesta categoria." : "As primeiras publicações da ENAT TV aparecerão aqui após a aprovação editorial."}</div>}</div>
      </section>
